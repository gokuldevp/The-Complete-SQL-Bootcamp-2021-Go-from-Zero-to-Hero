-- Assignment 2 -  From database exercises and schema cd.
...
is less than 1/50th of the monthly maintenance cost? Return the facid, facility name, 
...
/*6. How can you retrieve the details of facilities with ID 1 and 5? Try to do it without using 
the OR operator.*/
SELECT * FROM cd.facilities
WHERE facid IN (1,5);
...
SELECT f.facid, SUM(slots) AS slots FROM cd.facilities as f
...
/*12. Produce a list of facilities with more than 1000 slots booked. Produce an output table 
...
/*13. How can you produce a list of the start times for bookings for tennis courts, for the 
date '2012-09-21'? Return a list of start time and facility name pairings, ordered by the time.*/
SELECT starttime, name FROM cd.facilities as f
JOIN cd.bookings as b
ON f.facid = b.facid
WHERE to_char(starttime, 'yyyy-mm-dd') = '2012-09-21' AND f.facid IN (0,1)
ORDER BY starttime;

/*14. How can you produce a list of the start times for bookings by members named 'David Farrell'?*/
SELECT starttime FROM cd.members AS m
JOIN cd.bookings AS b
ON b.memid = m.memid
